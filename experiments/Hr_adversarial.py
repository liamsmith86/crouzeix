...
def main():
    ...
    with open(fname, "w") as fh:
        ...
        fh.flush()
        print(
            f"[n={n} t={t} s={seed}] init: ReC={best_r:+.6f} K={cur['K']:.5f}",
            flush=True,
        )
        for it in range(iters):
            ...
            if r2 is not None and r2["C"].real < best_r:
                ...
                fh.flush()
                print(
                    f"[n={n} t={t} s={seed}] it={it}: ReC={best_r:+.6f} "
                    f"K={cur['K']:.5f} s_phase={cur['s_phase']:+.5f}",
                    flush=True,
                )
            else:
                step = max(step * 0.85, 0.02)
    print(
        f"DONE n={n} t={t} s={seed}: min ReC = {best_r:+.8f} "
        f"(K={cur['K']:.5f}, s_phase={cur['s_phase']:+.5f})",
        flush=True,
    )
