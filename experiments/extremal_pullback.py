...
class Pullback:
    ...
    def extremal_data(self, A, alphas, T=None):
        ...
        ri = np.array([self.dpsi(a) / blaschke_prime_at_zero(alphas, i)
                       for i, a in enumerate(alphas)], dtype=complex)
        ...
        rhs = 2 - c_val / 2
        return dict(K=K, c=c_val, C=C, G=G, W=W, beta=beta, diag=diag,
                    lhs=lhs, rhs=rhs, slack=rhs - lhs, x0=x0, u0=u0)
...
if __name__ == "__main__":
    ...
    for eps in [0.0, 0.05, 0.1, 0.2]:
        ...
        for trial in range(4):
            ...
            print(f"eps={eps} n={n} t={trial}: K={d['K']:.6f} c={d['c']:.6f} "
