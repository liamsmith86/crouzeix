...
"""
import numpy as np
from mpmath import mp, mpf, sqrt as msqrt, pi as mpi, ellipk
...
def invariants(S):
    ...
    G = Ap.T @ Ap
    p = np.array([[np.trace(Q[i].T @ G @ Q[j]) for j in range(2)] for i in range(2)]).real
...
def stationarity_residual(S, alpha, p, delta):
    ...
    B1, Bp1 = B_and_prime(t1, alpha); B2, Bp2 = B_and_prime(t2, alpha)
    F1, F2 = B1/e1, B2/e2
    K2, T, Ssqrt = closed_K2(F1, F2, p, delta)
    T1 = 2*(p[0,0]*F1 + p[0,1]*F2); T2 = 2*(p[1,1]*F2 + p[0,1]*F1)
...
def rho_terms(S, alpha):
    ...
    t1, t2 = complex(S.tau[0]).real, complex(S.tau[1]).real
    B1, _ = B_and_prime(t1, alpha); B2, _ = B_and_prime(t2, alpha)
...
if __name__ == "__main__":
    cases = [(2.0,1.2,1.6,0.08), (1.8,1.0,1.4,0.15), (1.5,2.0,0.9,0.12),
             (2.2,0.9,1.3,0.10), (1.2,1.7,2.1,0.06)]
