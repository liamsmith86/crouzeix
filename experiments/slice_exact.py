...
class Slice:
    def __init__(self, a1, a2, a3, c):
        ...
        q = self.c**2 * t1 * t3
        disc = sqrt(p * p - 4 * q)
        self.e1 = sqrt((p + disc) / 2)
        self.e2 = sqrt((p - disc) / 2)
        ...
        for _ in range(220):
            mid = (lo + hi) / 2
            if ellipk(1 - mid) / ellipk(mid) > target:
                lo = mid
            else:
                hi = mid
        self.m = (lo + hi) / 2
    ...
    def psi_data(self, w):
        ...
        return (
            z,
            self.f / phip * 1 / self.f * self.f,
        )  # psi'(w) = 1/phi'(z); phi'(z)=phip/f
    ...
    def eval_alpha(self, alpha):
        ...
        def blaschke_value(w):
            return w * (w * w - alpha * alpha) / (
                1 - alpha * alpha * w * w
            )
        ...
        F1, F2 = (
            blaschke_value(t1) / e1,
            blaschke_value(t2) / e2,
        )  # F(e_j^2)
        Ap, Am = self.blocks()
        ...
        vals = [
            F1
            if abs(eigenvalue - E[0]) < abs(eigenvalue - E[1])
            else F2
            for eigenvalue in lam
        ]
    ...
    def rho_at(self, alpha):
        ...
        g1 = g0(self.e1, self.tau[0])
        g2 = g0(self.e2, self.tau[1])
        # rho = Re<(g0 f0)(A)x0,x0>: g0f0 even: H(e_j^2) = B(tau_j) g_j
        H1, H2 = (
            blaschke_value(self.tau[0]) * g1,
            blaschke_value(self.tau[1]) * g2,
        )
        Ap, Am = self.blocks()
...
if __name__ == "__main__":
    for a1, a2, a3, c in [
        ...
        print(
            f"a=({a1},{a2},{a3}) c={c}: alpha={al:.8f} K={K2:.6f} rho={rho:+.8f} "
            f"g(e1)={g1.real:+.6f}{g1.imag:+.1e}j g(e2)={g2.real:+.6f}{g2.imag:+.1e}j diag={diag:.1e}"
        )
        print(
            f"   tau1={complex(S.tau[0]).real:.6f} tau2={complex(S.tau[1]).real:.6f} "
