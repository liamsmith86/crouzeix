...
def eval_config(b_ell, zs, ams, bms, cp, N=1024, want_parts=False):
    ...
    gp_at = (
        np.array([np.sum(np.conj(pv) * zp / (z - zj) ** 2) for zj in zsa])
        * dt
        / (2j * np.pi)
    )
...
def main():
    ...
    def unpack(x):
        o = 0
        zs = x[o : o + NP] * 0.9 + 1j * x[o + NP : o + 2 * NP] * 0.9 * b_ell
        o += 2 * NP
        am = x[o : o + NP] + 1j * x[o + NP : o + 2 * NP]
        o += 2 * NP
        s = np.sum(am)
        if abs(s) < 1e-10:
            return None
        am = am / s
        if second:
            bm = x[o : o + NP] + 1j * x[o + NP : o + 2 * NP]
            o += 2 * NP
        else:
            bm = np.zeros(NP, dtype=complex)
            o += 0
    ...
    with open(f"scalar_adv_{tag}.jsonl", "w") as fh:
        for t in range(restarts):
            ...
            rec = dict(
                trial=t,
                **{
                    k: (v if not isinstance(v, complex) else [v.real, v.imag])
                    for k, v in r.items()
                },
            ...
            if r["viol"] > best:
                best = r["viol"]
                print(
                    f"[{tag}] t={t}: viol={r['viol']:+.6f} lhs={r['lhs']:.5f} "
                    f"rhs={r['rhs']:.5f} c={r['c']:.5f} numin={r['numin']:.2e}",
                    flush=True,
                )
