...
"""
import numpy as np
from mpmath import mp, mpf, mpc, ellipk, ellipf, ellipfun, asin, sin, sqrt, atanh, findroot
...
class EllipseMap:
    ...
    def dphi(self, z):
        u = (2 * self.K / mp.pi) * asin(z)
        return self.sk * cn(u, self.m) * dn(u, self.m) * (2 * self.K / mp.pi) / sqrt(1 - z ** 2)
...
def f_of_A(fm1, fp1, h):
    ...
    return np.array([[complex(fm1), complex(h) * (complex(fp1) - complex(fm1))],
                     [0.0, complex(fp1)]], dtype=complex)
...
def sandbox_eval(em, alpha):
    ...
    return dict(K=K_norm, C=C, W=W, beta=beta, q=q, s_phase=s_phase, diag=diag,
                z1=z1c, alpha=al)
...
def find_extremal_alpha(em, restarts=12, seed=0):
    ...
    for x0 in starts:
        res = minimize(negK, x0, method="Nelder-Mead",
