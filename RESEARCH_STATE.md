...
1. **Prove or falsify L147's reflected-ideal induction.**  Recenter the
   arbitrary-amplitude Weierstrass and Stein recurrences at L123's exact
   equality factor/metric.  Prove that the optimized envelope lies in the
   square of the convergent ideal generated by `r=c^L` and `w_k=c^ku_k`.
   Separate the dual preparation step from the primal critical-equation
   step.  L143's degree-two calculation and the 180-record probe are
   evidence only; do not promote them again without the all-orders
   induction.
