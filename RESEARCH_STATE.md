...
- There is now one live mathematical frontier: A194 at the complete
  ...
  reintroduce an odd weighted face.  The sole remaining envelopes are
  lower even two-sided radiality and active associated cyclic radiality
  through index `k+2`.  Exact grades one through six satisfy the
  stronger support statement, but that finite fact is not the missing
  induction.
...
- The complete frontier factors explicitly through
  ...
  vanishes as well.
- **Still open:** lower even two-sided radiality and active associated
  cyclic radiality through `Q_(k+2)`.  These are now the only two
  L276 support envelopes; do not compute more odd grades.
...
- **Still open:** prove the lower even radial envelope and active
  cyclic-radial envelope from L243/L251/L258/L272.  Do not grind the
  stronger literal no-extra-word theorem unless it falls out
  automatically.
...
1. **A194/A213/A216--A224: prove the two remaining cyclic support envelopes.**
   ...
   literal no-extra support is unnecessary.  L277 now proves every
   lower odd face vanishes by factoring its first frontier through the
   next delay ideal; do not recompute odd grades.  Prove only that
   lower even faces are two-sided radial and that the active associated
   difference is cyclically radial through index `k+2`.  Exact grades
   one through six satisfy the stronger support statement but are not
   an induction.
   L269 disproves the
   ...
   even and active L276 envelopes directly from the
   first-visit/model-kernel assembly.
   ...
   `D_ret=I−R_PZ_ret`, including the lower even vanishings (L277 has
   closed every lower odd face), by Hardy index:
