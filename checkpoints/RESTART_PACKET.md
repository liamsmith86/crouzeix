...
## Current general Crabb frontier (2026-07-26): after L309 / A256
...
## Next five concrete actions (refreshed after L309 / A256)
1. Construct the hereditary prepared affine recurrence starting from
   L306's all-series initial-copy energy
   `q=−Psi(Delta*XS+S*XDelta+Delta*XDelta)
      −(C*H+H*C+C*C)`.
   L308 proves raw odd parity false at quintic, while L309 proves that
   cubic/quartic preparation plus complete quintic lower
   neutralization still does not make the odd coefficient a global
   response.  For `Bcal_r=[B_1 ... B_r]`, seek a generating rule
   `U_(2r+1)=M_S(C_(2r+1))
      +X_r Bcal_r*+Bcal_r X_r*`
   with bounded analytic/Smith-divisible columns.  At even orders
   isolate the new direct Gram and factor the remaining cost through
   `Bcal_(r−1)`.  Use L307's positive
   ...
   bridge-module stability are already closed.  Do not impose global
   odd response, factor raw `q`, start an unrelated fifth/seventh
   march, enumerate L305 roots, discard `−C*C`, split load-bearing
   cancellations, or assume `I intersect [A,A]=[A,I]`.
...
## Paste-ready continuation instruction (current after L309/A256)
"Continue the Crouzeix campaign in `/home/liam/Downloads/crouzeix` as
the sole repository-writing agent.  Read L197, L220, L227, L283--L309,
