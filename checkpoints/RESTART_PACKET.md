...
where the free boundary-row Hessian is universally
`8||x_mid||²+(8/3)|x_L|²`.  If `J_C(h)` is the scalar norm Hessian
with moving inner tangent, then

`F_C(x)−4J_C(h)`

is exactly the sum of two endpoint-matching squares with weights
`diag(1,...,1,3)` and `diag(3,1,...,1)`.  The residual map in
`(x,h)` is bijective, so
...
`O_Ck−O_eq(Wu)=sum G_j eta_j`; hence
`delta+d asymp Delta_k`.  The live step is now the finite
L197/L321/Jensen/circular/elliptic merger, retaining the actual
direction until its responses are paid and keeping L318 separate.
General L193 disk points are not assumed nilpotent; monomial rigidity
is used only on the explicit nilpotent specialization.
...
L297, every exact disproved route has a banked certificate, all 260
...
pass all 310 experiment modules.  L290--L323 are explicitly scoped to
...
A259 then tests the natural constant-one endpoint-word Gram bound.
All deterministic words through length ten and wider stress tests
pass, but it is deliberately not L312: the proposed lossless-circuit
proof has an unresolved residual-state block after mixed gate
...
L313/A261 then restores the correct weight for the whole finite
physical sector.  Keeping `J=(I+F)S*(I+E)` intact, an exact
...
branch, staging conflict, or untracked artifact existed, and L248/L249
...
- `proof/gau_wu_boundary_observation.md` (L341),
  ...
  identification of the canonical observation kernel.  It is an
  equivalent diagnostic for L342's common sign, not a separate task.
...
- `proof/repeated_crabb_endpoint_word_gram.md` (A259 candidate, not
  L312): exact forward-energy reformulation, numerical evidence, and
...
- `proof/repeated_crabb_weighted_endpoint_polynomial.md` (L313/A261):
  exact wandering-chain recursion for the physical reverse edge,
  weighted factorization of every causal `S,J` polynomial, and the
  direct-map corollary.
- `experiments/repeated_crabb_weighted_endpoint_polynomial.py`:
  constructive word factors, complete direct-map jets, exhaustive
...
not close grouped elliptic weight matching.  L313 does prove the
weighted factorization for every finite physical word in
...
minimize over another direction before paying its responses.  Assemble
the finite L197/L321 flag and use L326/L192 on every split block.
Keep L199's split face and L318's elliptic margin separate.
L319/A267
