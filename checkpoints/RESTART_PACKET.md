...
## Current general Crabb frontier (2026-07-25): after L270 / A217
...
- `proof/repeated_crabb_toeplitz_return_rigidity.md` (L266): exact
  ...
  form/depth and all lower vanishings remain open.  L270 makes a
...
## Next five concrete actions (refreshed after L270 / A217)
1. **Close A213's leakage-ideal placement.**  L268 rejects the
   one-shot operator congruence, so expand L258's complete renewal
   with both L251 port orientations and prove that every active
   tail-dependent trace term lies in the two-sided ideal generated by
   `L_B`, with no one-sided `B#` or copy-dependent renewal denominator.
2. **Derive L269's shallow-plus-deep-divergence structure.**  In L264's port-isolated
   ...
   a trace-zero deep Stein divergence plus
   `P_kL_B Psi_phys(L,L*)L_BP_k`, with `Psi_phys` copy-scalar and
   either Laurent or of unilateral boundary depth at most `k`.
   L270 already forces the radial polynomial moments `p_k(1)=0`,
   `p_k'(1)=−1` and then closes its trace.  Do not compute the
   coefficients.  Prove the structural support, the fixed
   `R_2−R_1` and deep-divergence terms, and all lower vanishings.
   Do not assume away a copy-dependent insertion, do not delete the
   deep remainder, and perform the split only after both contour
   integrations.
...
Read the newest L258--L270 sections of `RESEARCH_STATE.md`, the
corresponding proof notes, and A194/A204--A217.  The live gate is the
...
vanishings.  Treat A213's two-sided leakage-ideal placement and the
