...
| A14 | F | Complete-2 similarity SDP for the elliptic slice | ACTIVE — PROMISING | For `T=φ(A)`, minimize `t` with `I≤P≤tI`, `T*PT≤P`. All 80 random cases have `t<4` (max 3.9264). A uniform analytic `P` would prove the whole slice and avoid phase-by-phase H-r |
