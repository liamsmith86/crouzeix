...
| A16 | F | Eliminate the contraction LMIs with Stein defects and classify coupled KKT rank faces | ACTIVE — EXACT 3-PARAMETER FACE | L27 parameterizes every contraction metric by two PSD 2×2 defects through a Szegő kernel. Complementary slackness reduces any hypothetical optimum above four to rank-one/rank-one or rank-one/full coupled faces, i.e. two defect directions and one relative scale. Arbitrary defect choices can be ill-conditioned; the remaining task is to exclude a KKT minimizer above four using the exact conformal relation. `proof/slice_coupled_defects.md` |
| A17 | F/B | Convert the rank-one/full coupled face to an odd two-node Blaschke block | ACTIVE — SQUARE DISTORTION BOUND LEFT | L28 identifies the generic sign-separated dual ratio with `sup_a ||B R_a(CB)||²`; the parity-swapped lower block is proved ≤2. L30 replaces L29's quartic determinant by signed equal-diagonal quadratics, closes one sign, and removes orientation and Blaschke parameter. L31 factors the remaining discriminant exactly as `16rg²p(1−c²)²(1−d²)−(Qr−S)²`, reducing the face to a sharp two-variable distortion inequality at two cubic-envelope endpoints. `proof/slice_odd_block_reduction.md` |
