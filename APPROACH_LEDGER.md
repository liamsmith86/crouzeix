...
| A17 | F/B | Convert the rank-one/full coupled face to an odd two-node Blaschke block | ACTIVE — 2-VARIABLE DISCRIMINANT LEFT | L28 identifies the generic sign-separated dual ratio with `sup_a ||B R_a(CB)||²`; the parity-swapped lower block is proved ≤2. L30 replaces L29's quartic determinant by signed equal-diagonal quadratics: one sign is closed by concavity plus L26, while the other needs only `4AC−B²≥0` on its interior-vertex branch at two cubic-envelope endpoints. Orientation and Blaschke parameter are eliminated. `proof/slice_odd_block_reduction.md` |
