...
| A17 | F/B | Convert the rank-one/full coupled face to an odd two-node Blaschke block | ACTIVE — LOW-NOME SQUARE BOUND LEFT | L28 identifies the generic sign-separated dual ratio with `sup_a ||B R_a(CB)||²`; the lower block is proved. L30 removes orientation/Blaschke parameter; L31/L32 give the cancellation-free endpoint residual. L33 closes `c≥12599/20000`; L34 centers the sharp ridge; L35 exactly closes its singular core `c≤1/20, |p−p*|≤4c⁴`. Certify the remaining low-nome complement. `proof/slice_odd_block_reduction.md` |
