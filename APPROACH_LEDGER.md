...
| A18 | F/B | Convert rank-one/rank-one to a matrix-valued transfer | ACTIVE — EXACT REDUCTION | Two spectral Gram rotations give `R_{a,b}(T)=−S⁻¹(T−A)(I−AT)⁻¹S`. Its modal node function is `2×2` rational inner with determinant `(ab−z²)/(1−abz²)`. Default rank-one/rank-one SDP values are reproduced sharply. Boundary/scalar reductions do not control the genuinely matrix-valued interior. `proof/slice_coupled_defects.md`; `experiments/slice_rank_one_transfer.py` |
