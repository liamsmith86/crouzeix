...
| A22 | F/B | Prove the full polynomial transfer core by projective principal-minor charts | **ACTIVE — EXACT REDUCTION, CONTINUOUS-`c` CERTIFICATE OPEN** | L51 reduces PSD to one `3×3` minor and the determinant. With `P=p²,t=1−o`, exact orders 3 and 4 yield two projective charts that pass full cubic-envelope Bernstein tests at every fixed `c` tested. High-order directed Taylor boxes correctly stall at further moving zero intersections (`v=0,|b|=1` and the square curve at `P=1,o=0`). Center/factor those intersections; do not revive Frobenius, rational-majorant, or orientation-endpoint shortcuts, all of which were falsified. `proof/slice_core_projective_reduction.md` |
