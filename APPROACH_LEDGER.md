...
| A17 | F/B | Convert the rank-one/full coupled face to an odd two-node Blaschke block | ACTIVE — LOW-NOME SQUARE BOUND LEFT | L28 identifies the generic sign-separated dual ratio with `sup_a ||B R_a(CB)||²`; the parity-swapped lower block is proved ≤2. L30 removes orientation and Blaschke parameter. L31/L32 factor the remaining discriminant into a cancellation-free `4[rF₁F₂−g²D²]` at two cubic-envelope endpoints. L33 closes the whole high-nome range `c≥13/20` by rectangle convexity, so only `0<c<13/20` remains. `proof/slice_odd_block_reduction.md` |
