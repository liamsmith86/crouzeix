...
| A27 | F/G | Resolve the higher-order `p=3` equality quotient with formal Riemann and Stein series | **EVERY CANONICAL STRAIGHT RAY PROVED (L67/L68); UNIFORM SLICE ACTIVE** | Exact normal-angle reparameterization and rank-one Stein Gramians give `4−171ε⁶/4096+O(ε⁷)` on the pure residual mode-one ray. On all canonical straight rays with mode-two coordinate `ω≠0`, the quartic is `−4|ω|⁴−31|ζ|²|ω|²/8`. A weighted analysis of `ω=O(εζ)` and an exact local affine-unitary slice with uniform coupling to the negative second-order directions are still needed for a punctured-neighbourhood theorem. `proof/p3_crabb_higher_order.md`. |
