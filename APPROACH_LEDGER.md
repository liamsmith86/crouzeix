...
| A24 | F/G | Resolve the zero-Jensen-gap face at second order | **FINITE CONFORMAL+SDP REDUCTION PROVED; SIGN OPEN (L62)** | Second Hermitian eigenvalue perturbation, the normal-angle correction, and a Schwarz integral give finite `G,H`; first-order complementarity then reduces all second-order PSD conditions to three affine block LMIs in `X,Y,e`. An independent five-point conformal fit, two resolutions, and two solvers reproduce 12 nonlinear quadratic coefficients; all are negative, with two nearly flat structured `p=3` cases. Next dualize the finite SDP and prove its sign. `proof/general_similarity_second_order.md` |
...
| A27 | F/G | Resolve the higher-order `p=3` equality quotient with formal Riemann and Stein series | **EVERY CANONICAL STRAIGHT RAY PROVED (L67/L68); UNIFORM COUPLING ACTIVE** | Exact normal-angle reparameterization and rank-one Stein Gramians give `4−171ε⁶/4096+O(ε⁷)` on the pure residual mode-one ray. On all canonical straight rays with mode-two coordinate `ω≠0`, the quartic is `−4|ω|⁴−31|ζ|²|ω|²/8`. L69 now supplies the exact local slice; its weighted coupling to the negative directions remains before a punctured-neighbourhood theorem. `proof/p3_crabb_higher_order.md`. |
| A28 | F/G | Build the exact `p=3` affine-unitary normal slice and couple its negative directions to the residual quotient | **EXACT SLICE PROVED (L69); WEIGHTED SIGN ACTIVE** | The orbit-normal slice is the seven-real-parameter matrix `E(z,w,s,v)` in L69. L63 restricts to `−8s²−21|v|²/4`, so `(z,w)` are the complete flat coordinates and `(s,v)` the complete negative coordinates. This removes orbit ambiguity and reduces a full neighbourhood theorem to one finite weighted expansion. `proof/p3_crabb_local_slice.md`. |
