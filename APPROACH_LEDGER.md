...
| A27 | F/G | Resolve the higher-order `p=3` equality quotient with formal Riemann and Stein series | **PURE MODE-ONE RAY PROVED (L67); MIXED FACE ACTIVE** | Exact normal-angle reparameterization constructs `φ_ε(C₃+εE)` through order six. A rank-one-defect Gramian has condition `4−171ε⁶/4096+O(ε⁷)` on the pure residual mode-one ray, proving strict local descent there. The apparent quartic on nonzero mode-two mixtures remains to be derived; L67 is not yet a full `p=3` neighbourhood theorem. `proof/p3_crabb_higher_order.md`. |
