...
| A22 | F/B | Prove the full polynomial transfer core by projective principal-minor charts | **ACTIVE — L51/L52, CONTINUOUS-`c` CERTIFICATE OPEN** | L51 reduces PSD to one `3×3` minor and the determinant. With `P=p²,t=1−o`, exact orders 3 and 4 yield two projective charts that pass full cubic-envelope Bernstein tests at every fixed `c` tested. A second exact order-one blow-up resolves `v=0,|b|=1`. L52 proves both orientation faces analytically and shows the apparent square at `P=1,o=0` has no interior zero: its scalar factor `G_x` is strictly negative. Normalize interval arithmetic by these factors; do not revive Frobenius, rational-majorant, or block-energy orientation-endpoint shortcuts, all falsified. `proof/slice_core_projective_reduction.md` |
