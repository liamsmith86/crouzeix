...
| A18 | F/B | Convert rank-one/rank-one to a matrix-valued transfer | ACTIVE — EXACT REDUCTION | Two spectral Gram rotations give `R_{a,b}(T)=−S⁻¹(T−A)(I−AT)⁻¹S`. Its modal node function is `2×2` rational inner with determinant `(ab−z²)/(1−abz²)`. L41 converts the norm test to a quartic polynomial block LMI and proves both diagonal blocks PSD; only its Schur coupling remains. L42 closes `c≥2^(−2/3)` directly from the inner-function model. L43 square-completes the remaining Schur coupling into an explicit small-gain matrix; the stronger four-block energy inequality L44 survives global searches. Default rank-one/rank-one SDP values are reproduced sharply. `proof/slice_coupled_defects.md`; `experiments/slice_rank_one_transfer.py` |
