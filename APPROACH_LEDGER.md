...
| A22 | F/B | Prove the full polynomial transfer core by projective principal-minor charts | **ACTIVE — L51--L55, GLOBAL `c` COVER OPEN** | L51--L53 give the complete exact chart hierarchy and close the orientation faces. L54 rigorously covers `c∈[.01,.0144]` with two Arb/directed-Bernstein boxes, all final charts and signs. L55 resolves the low-nome cancellation exactly: determinant order `c^9`, minor order `c^2`, then exceptional forms `1536(A+3U)` and `192[8A₂+24U₂+3V²+18(B−2σ)²]`; the last positive-sign ridge starts with `46080c⁴`. Finish its interval remainders while extending the compact boxes. No negative core has appeared. `proof/slice_core_projective_reduction.md` |
