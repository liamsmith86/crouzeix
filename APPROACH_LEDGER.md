...
| A15 | F | Decompose L20 into modal-block and parameter-boundary theorems | PARTIAL — PROVED PIECES | L23 gives the sharp nome bound; L24 proves one modal block `||C||≤2`; L25 proves the full similarity bound on the sharp `c→0` weighted-shift face. The upper block leaves the genuine conformal displacement `(p−r)/c`, and coupled dual witnesses remain. `proof/slice_boundary_theorems.md` |
