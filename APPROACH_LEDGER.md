...
| A21 | F/B | Resolve the zero-node transfer ridge with a sharper nome estimate | **PARTIAL — L49/L50 PROVED** | Jacobi's first two product factors give `k≤4c/(1+4c²)` for `c≤1/2` by a 24-coefficient exact Bernstein check. On `p=0`, BE reduces to one scalar inequality; `k≤1` closes its complete `c≥1/2` half with 125 exact Bernstein coefficients. The low half has the curved equality ridge `b=1,a=2c`; use a centered/blow-up chart rather than raw boxes. `proof/slice_coupled_defects.md` §6.4; `experiments/slice_sharp_nome_certificate.py` |
