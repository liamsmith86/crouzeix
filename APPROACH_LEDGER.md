...
| A22 | F/B | Prove the full polynomial transfer core by projective principal-minor charts | **ACTIVE — L51--L54, GLOBAL `c` COVER OPEN** | L51--L53 give the complete exact chart hierarchy and close the orientation faces. L54 upgrades the corrected shared-coordinate Taylor scheme to a rigorous Arb/directed-Bernstein theorem on `c∈[.01,.012]`, all five final charts and both signs. Twenty-percent nome boxes are viable; extend them adaptively over the compact range. The remaining singular task is a normalized `c→0` chart, where the determinant has two extra cancelling powers and sharp ridges near `b≈2c`. No negative core has appeared. `proof/slice_core_projective_reduction.md` |
