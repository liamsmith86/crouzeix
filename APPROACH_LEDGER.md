...
| A22 | F/B | Prove the full polynomial transfer core by projective principal-minor charts | **ACTIVE — ONLY POSITIVE COMPACT TAIL OPEN** | L56 bridges L55 to L54, so every chart/sign is proved continuously on `0≤c≤.020736`. L57 certifies the negative sign through `.63` (overlapping L42) and the positive sign through `c+=.5566585294072849…`; only `c+<c<2^(−2/3)` remains. L58 factors the sharp positive edge `a=b=1` into even powers and proves the inward first variation nonnegative. The next certificate must retain correlated nome/spatial cancellation in the quadratic-and-higher deficit remainder; its exact global l1 norm `41235531913` is safe but far too coarse. Do not return to tolerance-based or unbounded raw subdivision. `proof/slice_core_projective_reduction.md`; `experiments/slice_positive_face_audit.py` |
