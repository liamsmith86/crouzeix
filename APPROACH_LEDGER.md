...
| A17 | F/B | Convert the rank-one/full coupled face to an odd two-node Blaschke block | ACTIVE — ONE SCALAR DETERMINANT LEFT | L28 uses an orthogonal colligation to identify the generic sign-separated dual ratio with `sup_a ||B R_a(CB)||²`; the parity-swapped lower block is proved ≤2. L29 reduces the upper block exactly to `N(c,p,H(p),a)≥0`, independent of orientation. A rigorous two-sided cubic envelope for `H` contains the whole numerical target and exposes the sharp `p=1/2, a∼3c` ridge. `proof/slice_odd_block_reduction.md` |
