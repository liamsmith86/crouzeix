...
| A19 | F/G | Test L21's contraction-similarity route on general matrices before further slice certification | **NUMERICALLY SURVIVES; SCALAR-SHIFT SHORTCUT CLOSED** | A certificate-gated sweep accepted 102/120 varied complex matrices (`n=3..8`) and found no `t*>4`; max `3.8399296`. Near-Crabb and triangular offset ladders approach four only from below. Hartz–McCarthy gives `inf_beta||theta+beta I||cb=||theta||cb` above norm one, while the Crouzeix–Palencia correction is operator-valued; hence scalar-shift search is an exact restatement, not a bridge. An order-three Toeplitz SDP numerically excludes every positive-state scalarization on one dense 3×3 case (stable 512→8192 and across SCS/Clarabel). L21 remains credible finite evidence for a potentially stronger complete-bounded statement. `proof/general_similarity_probe.md` |
