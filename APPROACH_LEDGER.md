...
| A22 | F/B | Prove the full polynomial transfer core by projective principal-minor charts | **ACTIVE — L51--L53, FINITE `c` COVER OPEN** | L51 reduces PSD to one `3×3` minor and the determinant. Main orders 3/4 and nested minor orders 1/1 give a complete three-level projective chart hierarchy, now regenerated exactly by `slice_projective_core.py`. L52 proves both orientation faces and rules out the suspected moving zero. Correcting a zeroth-Taylor-term bug and retaining `c` as a shared Bernstein coordinate gives successful continuous local boxes for every final chart. The remaining certificate work is a memory-bounded compact nome cover plus a normalized `c→0` chart; no negative core has appeared. `proof/slice_core_projective_reduction.md` |
