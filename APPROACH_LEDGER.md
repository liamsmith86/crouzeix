...
| A25 | F/G | Eliminate the single-Crabb second-order SDP in closed form | **PROVED FOR `p=3,4` (L63–L64); GENERAL `p` OPEN** | At `p=3`, the optimum is `−2(Re(E01−E12))²−21|E20|²/4`. At `p=4`, it is an explicit five-term negative sum of squares of rank eight. Regenerating SymPy certificates start from all 18/32 real perturbation coordinates, verify the exact support eigenpairs, rebuild the conformal coefficients, eliminate the metric variables, and prove both identities without fitted coefficients. This gives nonpositive second-order change in every direction at the two smallest nontrivial Crabb blocks and identifies their equality spaces. Next seek a general-`p` factorization rather than grind one size at a time, while classifying the equality directions. `proof/general_similarity_second_order.md` §§8–9. |
